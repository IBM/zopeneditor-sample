...
```bash
cd zcodescan/customrules/COBOL/cobol-customrules
mvn clean package
```
...
**Implementation**: See [CobolProgramIdRule.java](src/main/java/com/ibm/zcodescan/cobol/api/impl/CobolProgramIdRule.java)
...
1. **Define Rule Metadata** in `../rules-domains.yaml`
2. **Activate the Rule** in `../sam-rules.yaml`
3. **Configure JAR Location** in `../../zapp.yaml`
4. **Build the JAR**: `cd zcodescan/customrules && mvn clean package`
