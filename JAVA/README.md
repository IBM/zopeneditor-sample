...
- `target/zcodescan-1.0-SNAPSHOT.jar` (JAR file with the custom code class)

### 2. Configure ZCodeScan

The workspace is already configured in `../../zapp.yaml`:
  ...
  - name: zcodescan
    ...
    settings:
      customRuleJars:
        - type: local
          locations:
            - "JAVA/zcodescan/target/zcodescan-1.0-SNAPSHOT.jar"
```

### 3. Test the Rule

1. Open `../../COBOL/SAM1.cbl`
...
│   └── JavaProgramIdRule.java                 # Example custom rule
└── target/
...
The included `JavaProgramIdRule` demonstrates:
...
**Implementation**: See [JavaProgramIdRule.java](zcodescan/src/main/java/com/ibm/zcodescan/cobol/api/impl/JavaProgramIdRule.java)
...
**Documentation**: [ZCodeScan Java API Documentation](https://ibmdocs-test.dcs.ibm.com/docs/en/developer-for-zos/17.0.x?topic=rules-zcodescan-java-api-documentation)
...
**API Reference**: [COBOL API Reference](https://ibmdocs-test.dcs.ibm.com/docs/en/developer-for-zos/17.0.x?topic=apis-cobol-api-reference)
...
6. **Issues** are returned to ZCodeScan and displayed in VS Code Problems panel
...
4. **Build the JAR**: `cd JAVA/zcodescan && mvn clean package`
...
5. **Build and Test**: `mvn clean package` then reload VS Code
